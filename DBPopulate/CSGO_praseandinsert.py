    ...
    with open('./yelp_user.JSON','r') as f:
        ...
        while line:
            ...
            try:
                ...
                cur.execute("INSERT INTO users (user_id, name, average_stars, fans, cool, tipcount, funny, useful, totallikes, lat, long)"
                       + " VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                         (data['user_id'],cleanStr4SQL(data["name"]), data["average_stars"], data["fans"], data["cool"], tipcount, data["funny"], data["useful"], 0, 0 , 0 ) )              
            except Exception as e:
...
def insert2Checkins(conStr):
    ...
    #connect to yelpdb database on postgres server using psycopg2
        ...
        while line:
            ...
            command = "INSERT INTO checkins (year, month, day, time, business_id) VALUES"
            idx = 0
            for checkin in checkin_date:
                if idx > 0:
                    command += ","
                command += " ('{0}', '{1}', '{2}', '{3}', '{4}')".format(checkin[0:4], checkin[5:7], checkin[8:10], checkin[11:19], data["business_id"])  
                idx += 1     
            try:
                cur.execute(command)       
                conn.commit()
...
#             data = json.loads(line)
#             try:
#                 cur.execute("UPDATE users SET useful = (%s) WHERE user_id = (%s)", 
#                          (data['useful'], data['user_id']) )              
#             except Exception as e:
#                 print("update users failed!", e, data, count_line)
#                 break
#             conn.commit()
...
# def updateUserYelpingSince(conStr):
#     #reading the JSON file
#     with open('./yelp_user.JSON','r') as f:
#         line = f.readline()
#         count_line = 0
...
#             data = json.loads(line)
#             try:
#                 cur.execute("UPDATE users SET yelping_since = (%s) WHERE user_id = (%s)", 
#                          (data['yelping_since'], data['user_id']) )              
#             except Exception as e:
#                 print("update users failed!", e, data, count_line)
#                 break
#             conn.commit()
